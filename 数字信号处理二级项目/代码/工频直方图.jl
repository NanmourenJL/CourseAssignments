...
function count_get()
    ...
    global input_signal
    global real_signal
    real_signal = sine_get(rand_get(1,2),rand_get(0.5,4 ),rand_get(0,2*pi),N,delta)+
                sine_get(rand_get(1,2),rand_get(  4,8 ),rand_get(0,2*pi),N,delta)+
                sine_get(rand_get(1,2),rand_get(  8,13),rand_get(0,2*pi),N,delta)+
                sine_get(rand_get(1,2),rand_get( 13,30),rand_get(0,2*pi),N,delta)
    noice_signal=sine_get(1,50,rand_get(0,2*pi),N,delta).*rand(N)
    ...
    iir_l=sqrt.(iir_s)                          #
    return mean(iir_lms),mean(iir_l);
end
...
h1 = histogram(a; facealpha=0.6)
hold("on")
h2 = histogram(b; facealpha=0.6)
hold("off")
legend(["IIR滤波器输出信号","自适应滤波器输出信号"])